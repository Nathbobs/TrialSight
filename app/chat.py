"""
RAG Chat — clinical trial Q&A powered by Qdrant + Groq Llama 3.3 70B.
Supports optional PDF upload for session-scoped document search.

render_chat_panel() renders a compact floating panel (used by main.py).
"""
...
def render_chat_panel() -> None:
    """Render the compact floating chat panel (bottom-right overlay)."""
    model = _get_embedding_model()
    qdrant_client = _get_qdrant_client()
    groq_client = _get_groq_client()

    # Panel header
    st.markdown("#### 💬 Clinical Trials Chat")
    ...
    if not st.session_state["messages"]:
        st.markdown(
            "**Try asking:**\n"
            "- What trials are studying Alzheimer's?\n"
            "- Show me Phase 3 cancer trials\n"
            "- Which trials are recruiting for diabetes?\n"
            "- What is NCT04280705?\n"
        )
    ...
    # Input form — st.form works inside containers; st.chat_input does not
    with st.form("chat_panel_form", clear_on_submit=True):
